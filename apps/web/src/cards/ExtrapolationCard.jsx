  ...
  return (
    <Card>
      <CardHead
        ...
        sub={`Carrying ${city.shortName}'s measured overnight-low trend (+${perDecade.toFixed(1)}°F per decade since ${windowStart}) forward in a straight line to ${HORIZON}. The shaded fan is only the statistical uncertainty in that historical slope — the same 95% moving-block bootstrap as the headline — and it widens the farther it reaches past the record.`}
      />
      ...
      <p className="text-sm mt-2 leading-relaxed" style={{ color: C.text }}>
        ...
        <span style={{ color: C.gold, fontFamily: DISPLAY }}>{at2050.toFixed(0)}°F (±{half.toFixed(0)}) by {HORIZON}</span>
        , up from ~{recent.toFixed(0)}°F over the last decade.
      </p>
