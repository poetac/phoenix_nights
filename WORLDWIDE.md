...
- **Phase B — first international slice (precomputed).** *Step 0 ✅: the GHCN-Daily
  ...
  its source station by verification first. **First city in progress: Sydney, AU**
  (`source:"ghcn"`, GSOY station ASN00066062, rural control Bathurst Agricultural
  ASN00063005, `units:"metric"`). The backend is source-aware now — `cities.py`
  carries a `source`/`primary_sid` split, `build_facts` + a new `build_series.py`
  take a GSOY path (trends ranked in °F, labels rendered in °C), and `check_cities`
  / `build_compare` skip non-ACIS cities so the US path is byte-identical. Assets are
  generated by the rebuild Action (egress); the front-end city entry + non-ACIS data
  path follow once they exist. Next: ~5 cities with dense networks and
